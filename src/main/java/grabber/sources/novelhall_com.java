...
public class novelhall_com implements Source {
    ...
    public List<Chapter> getChapterList() {
        ...
        try {
            ...
            Elements chapterLinks = toc.select("#morelist li a");
    ...
    public NovelMetadata getMetadata() {
        ...
        if (toc != null) {
            ...
            metadata.setAuthor(toc.select("meta[property=books:author]").attr("content"));
            String description = toc.select(".js-close-wrap").first().text().replace("back<<", "");
            metadata.setDescription(description);
            metadata.setBufferedCover(toc.select("meta[property=og:image]").attr("abs:content"));
            Elements tags = toc.select(".booktag .red");
