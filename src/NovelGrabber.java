...
public class NovelGrabber {
	...
	private void initialize() {
		...
		frmNovelGrabber.setTitle("Novel Grabber - 1.0");
		...
		frmNovelGrabber.getContentPane().add(tabbedPane);
				...
				automaticPane.setBorder(BorderFactory.createTitledBorder("Get multiple chapters"));
				tabbedPane.addTab("Automatic", null, automaticPane, null);
				
				JPanel allChapterPane = new JPanel();
				allChapterPane.setBounds(0, 0, 577, 407);
				...
				progressBar.setBounds(20, 366, 409, 30);
				...
				scrollPane.setBounds(20, 221, 532, 134);
				allChapterPane.add(scrollPane);
				
				JPanel chapterSelect = new JPanel();
				chapterSelect.setBounds(20, 109, 532, 45);
				...
				optionSelect.setBounds(20, 165, 532, 51);
				...
				createTocCheckBox.setToolTipText(toolTipStyle + "Will create a \"Table of Contents\" file which can be used to convert all chapter files into a single epub file in calibre.</p></html>");
						...
						singleChapterPane.setBounds(0, 406, 577, 100);
						...
						lblchapterURL.setBounds(20, 24, 100, 25);
						...
						JButton removeLinks = new JButton("Remove selected links");
						...
						removeLinks.setBounds(410, 50, 137, 25);
						...
						lblLinkSelect.setBounds(10, 50, 227, 25);
						...
						retrieveLinks.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent arg0) {
								if(manChapterListURL.getText().isEmpty() == true) {
									JOptionPane.showMessageDialog(frmNovelGrabber, "URL field is empty.", "Warning", JOptionPane.WARNING_MESSAGE);
						...
						manChapterContainer.setToolTipText(toolTipStyle + "Input chapter wrapping <div> selector following jsoup conventions. For example: .fr-view/.chapter-text etc for <div> class names. #mw-content-text/#chapter-wrapper for <div> id names. More info on jsoup.org</p></html>");
						...
						manSentenceSelector.setToolTipText(toolTipStyle + "Input html sentence wrapping. Usually \"p\" for <p>(paragraphs). Use \"*\" as a wildcard.</p></html>");
						...
						lblSaveLocation.setBounds(20, 428, 118, 25);
						manualPane.add(lblSaveLocation);
						
						manDestinationFolder = new JTextField();
						manDestinationFolder.setBounds(126, 430, 319, 25);
						...
						btnManBrowse.setBounds(455, 429, 112, 27);
						manualPane.add(btnManBrowse);
						
						JSeparator separator = new JSeparator();
						separator.setBounds(15, 464, 552, 5);
						...
						manCreateToc.setToolTipText("<html><p width=\"300\">Will create a \"Table of Contents\" file which can be used to convert all chapter files into a single epub file in calibre.</p></html>");
						...
						manChapterNumeration.setToolTipText("<html><p width=\"300\">Will add a chapter number infront of the chapter name. Helpful for ordering chapters which don't have a chapter number in their title.</p></html>");
						manChapterNumeration.setFont(new Font("Tahoma", Font.PLAIN, 11));
						manChapterNumeration.setBounds(236, 15, 149, 23);
						...
						btnManGrabChapters.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								...
								if(manChapterListURL.getText().isEmpty() == true) {
									...
									JOptionPane.showMessageDialog(frmNovelGrabber, "Save directory field is empty.", "Warning", JOptionPane.WARNING_MESSAGE);
									manDestinationFolder.requestFocusInWindow();
								}
								else if(manChapterContainer.getText().isEmpty() == true) {
									JOptionPane.showMessageDialog(frmNovelGrabber, "Chapter container selector is empty.", "Warning", JOptionPane.WARNING_MESSAGE);
									manChapterContainer.requestFocusInWindow();
								}
								else if(manSentenceSelector.getText().isEmpty() == true) {
									JOptionPane.showMessageDialog(frmNovelGrabber, "Sentence selector is empty.", "Warning", JOptionPane.WARNING_MESSAGE);
									manDestinationFolder.requestFocusInWindow();
								}
								else if((manFileType.getSelectedItem().toString() == "TXT") && (manCreateToc.isSelected() == true)) {
									JOptionPane.showMessageDialog(frmNovelGrabber, "Cannot create Table of Contents page from txt files.", "Warning", JOptionPane.WARNING_MESSAGE);
									manDestinationFolder.requestFocusInWindow();
								}
								else if((!manDestinationFolder.getText().isEmpty() == true) && (!manChapterListURL.getText().isEmpty() == true) &&
										(!manChapterContainer.getText().isEmpty() == true) && (!manSentenceSelector.getText().isEmpty() == true)) {
									try {
										manProgressBar.setStringPainted(true);
										if(manFileType.getSelectedItem().toString() == "TXT") {
											fetchChapters.manSaveChapters(manDestinationFolder.getText(), ".txt", manChapterNumeration.isSelected(),
													manChapterContainer.getText(), manSentenceSelector.getText());
										}
										else {
											fetchChapters.manSaveChapters(manDestinationFolder.getText(), ".html", manChapterNumeration.isSelected(),
													manChapterContainer.getText(), manSentenceSelector.getText());
										}
								...
								getAllChaptersBtn.addActionListener(new ActionListener() {
									public void actionPerformed(ActionEvent arg0) {
										...
										if(chapterListURL.getText().isEmpty() == true) {
											...
											JOptionPane.showMessageDialog(frmNovelGrabber, "Cannot create a Table of Contents file with .txt file type.","Warning", JOptionPane.WARNING_MESSAGE);									
										}
											...
											JOptionPane.showMessageDialog(frmNovelGrabber, "No chapter range defined.","Warning", JOptionPane.WARNING_MESSAGE);
										}
										else if((chapterAllCheckBox.isSelected() == false) && (!firstChapter.getText().matches("\\d+") || !lastChapter.getText().matches("\\d+"))) {
											JOptionPane.showMessageDialog(frmNovelGrabber, "Chapter range must contain numbers.","Warning", JOptionPane.WARNING_MESSAGE);
										}
											...
											JOptionPane.showMessageDialog(frmNovelGrabber, "Chapter numbers can't be lower than 1.","Warning", JOptionPane.WARNING_MESSAGE);
										}
										else if((chapterAllCheckBox.isSelected() == false) && (Integer.parseInt(lastChapter.getText()) < Integer.parseInt(firstChapter.getText()))) {
											JOptionPane.showMessageDialog(frmNovelGrabber, "Last chapter can't be lower than first chapter.","Warning", JOptionPane.WARNING_MESSAGE);
										}
